...
ここで変数の ``fun`` 変数は、``parent()`` 関数のリファレンスを保持しています。このリファレンスが紐付いている先の ``parent()`` 関数は未だ呼び出されていません。
...
ここで、``parent()`` 関数の内側でネストした二つの関数 ``son()`` と ``daughter()`` を定義しています。
このネストした二つの関数は直接（関数の外側から）呼びされることはなく、また外部の関数が呼び出されても自動的に実行されることはありません。
